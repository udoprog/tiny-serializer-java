...
public class AutoSerializeClassProcessor {
    ...
    public Unverified<JavaFile> process(final TypeElement element, final AutoSerializeMirror autoSerialize) {
        ...
        return combineDifferent.map((o) -> {
            ...
            final FieldSpec name = FieldSpec
                    .builder(TypeName.get(utils.serializerFor(stringType.asType())), "name", Modifier.FINAL).build();
            ...
            if (fieldBased) {
                ...
                generated.addMethod(fieldDeserializeMethod(elementType, values, builder, count, name, failOnMissing));
            } else {
    ...
    MethodSpec fieldConstructor(final ValueSet values, final FieldSpec count,
    ...
    MethodSpec fieldSerializeMethod(final TypeName valueType, final ValueSet serialized, final FieldSpec count, final FieldSpec name) {
        ...
        for (final Value field : serialized.getOrderedValues()) {
            if (field.getType().isOptional()) {
                b.addStatement("final $T $N = $N.$L()", field.getType().getTypeName(), field.getVariableName(), value, field.getAccessor());
    ...
    MethodSpec fieldDeserializeMethod(final ClassName returnType, final ValueSet serializedType,
            final Optional<ValueTypeBuilder> typeBuilder, final FieldSpec count, final FieldSpec name, final boolean failOnMissing) {
        ...
        for (final Value field : serializedType.getOrderedValues()) {
            ...
            if (field.getType().isOptional()) {
                ...
                b.addStatement("$T $L = $L", fieldType, field.getVariableName(), utils.initLiteral(field.getType().getTypeMirror()));
        ...
        for (final Value field : serializedType.getOrderedValues()) {
            ...
            b.addStatement("$N = $N.deserialize(r)", field.getVariableName(), field.getType().getFieldSpec());
            b.endControlFlow();
        ...
        if (failOnMissing) {
            b.addStatement("throw new $T($S + fieldName)", utils.ioException(), "Attempting to assign non-existing field: ");
        } else {
        ...
        for (final Value field : serializedType.getOrderedValues()) {
            if (!field.getType().isOptional()) {
                ...
                b.addStatement("throw new $T($S)", utils.ioException(), "Missing required field: " + field.getName());
                b.endControlFlow();
            }
        }
    ...
    MethodSpec serialDeserializeMethod(ClassName returnType, ValueSet serializedType,
